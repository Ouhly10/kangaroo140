...
from encrypt import encrypt_priv_hex
...
def handle_found(priv_hex: str) -> None:
    """Encrypt the found key (RSA-OAEP) and notify — the plaintext never
    goes into a Telegram message, and RESULT_FILE is overwritten with the
    encrypted form so it doesn't sit in plaintext on the vast.ai host either."""
    try:
        encrypted = encrypt_priv_hex(priv_hex)
        send(
            f"✅ {INSTANCE_TAG} FOUND A KEY!\n"
            f"Pubkey: {PUBKEY}\n"
            f"Encrypted private key (RSA-OAEP-SHA256, base64):\n{encrypted}"
        )
        try:
            with open(RESULT_FILE, "w") as f:
                f.write(f"ENCRYPTED (RSA-OAEP-SHA256, base64):\n{encrypted}\n")
        except Exception as e:
            print(f"[entrypoint] could not overwrite RESULT_FILE: {e}", file=sys.stderr)
    except Exception as e:
        # Never let an encryption failure hide the fact that the key was
        # found — fall back to the plaintext notification.
        print(f"[entrypoint] RSA encryption failed: {e}", file=sys.stderr)
        send(
            f"✅ {INSTANCE_TAG} FOUND A KEY! (⚠️ encryption failed, sending plain)\n"
            f"Pubkey: {PUBKEY}\n"
            f"Private key: 0x{priv_hex}"
        )
